...
module dbox(x=0, y= 0, z=0) {
  ...
  dz = d*.5; r = 2; // tall enough so dice don't fall out top
  translate([x, y, z]) 
  ...
  if (loc == 1)
    ...
    astack(4, d+.01, [0, 0, 0]) color("white") roundedCube(d,1);
  difference() 
  {
...
cw = 28; sy = [t0, w-cw-t0*2.2, t0*1.5];
if (loc != 0)
...
module pbox(xyz, t=t0) {
  ...
  translate(xyz) 
  atrans(loc, [[0,0,0], [-bxx+f/2 , 0, t0+p]])
  difference() {
